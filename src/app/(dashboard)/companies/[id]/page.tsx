    ...
    <div className="max-w-4xl space-y-7">
      ...
      <div className="border-b border-slate-800/80 pb-5">
        <div className="flex items-center gap-2 font-mono text-xs text-slate-400">
          <Link href="/companies" className="hover:text-white flex items-center gap-1 transition-colors">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 font-mono text-xs text-slate-400">
              Account ID: <span className="text-slate-300">{typedCompany.id}</span>
            </p>
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-750 bg-slate-900/90 p-5 shadow-sm space-y-4">
            ...
            {STAGES.map((s) => (
              <form key={s} action={updateCompanyStage.bind(null, id, s)}>
                <button
                  ...
                  className={`rounded border px-3 py-1 text-xs font-semibold capitalize transition-all disabled:opacity-40 ${
                    typedCompany.pipeline_stage === s
                      ? "border-amber-700/80 bg-amber-950/90 text-amber-200 shadow-[0_0_8px_rgba(245,158,11,0.15)]"
                      : "border-slate-750 bg-slate-950 text-slate-300 hover:border-slate-650 hover:text-white"
                  }`}
                >
                  {s}
                </button>
          ...
          <form action={toggleJdFormReceived.bind(null, id)} className="flex items-center gap-2.5 pt-3 border-t border-slate-800">
            <input
              id="jd_form_received"
            ...
            <button
              type="submit"
              className="ops-button-secondary ml-auto text-xs py-1 px-2.5 min-h-0"
            >
              Update
            </button>
          </form>
        ...
        <div className="rounded-lg border border-slate-750 bg-slate-900/90 p-5 space-y-4 shadow-sm">
          <div className="rounded border border-blue-800/60 bg-blue-950/40 p-3 text-xs font-mono text-slate-300">
            <p className="text-[10px] uppercase tracking-wider text-blue-400 font-bold">Escalation Chain</p>
            <p className="mt-1 text-slate-200">
              <strong className="text-amber-300 font-semibold">{assignedOwner?.name ?? "Unassigned JPC"}</strong> (JPC Owner)
              {" ➔ "}
              <strong className="text-blue-300 font-semibold">{assignedSupervisor?.name ?? "Unassigned Senior SPC"}</strong> (Supervisor)
            </p>
          </div>
      ...
      <div className="rounded-lg border border-slate-750 bg-slate-900/90 p-5 shadow-sm space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {contactRows.map((c) => (
            <div key={c.id} className="rounded-md border border-slate-800 bg-slate-950 p-3.5 shadow-inner">
        ...
        {/* Add Contact Form */}
        <form action={addCompanyContact.bind(null, id)} className="pt-3 border-t border-slate-800 grid grid-cols-2 sm:grid-cols-4 gap-2">
          <input name="title" placeholder="Title (Mr/Ms)" className="ops-input text-xs text-white" />
          <input name="full_name" placeholder="Full Name *" required className="ops-input text-xs text-white" />
          <input name="last_name" placeholder="Last Name" className="ops-input text-xs text-white" />
          <input name="hr_designation" placeholder="Designation" className="ops-input text-xs text-white" />
          <input name="email" type="email" placeholder="Email" className="ops-input text-xs text-white" />
          <input name="cc_email" type="email" placeholder="CC Email" className="ops-input text-xs text-white" />
          <input name="phone" placeholder="Phone" className="ops-input text-xs text-white" />
          <button type="submit" className="ops-button-primary text-xs justify-center">
            + Add Contact
          </button>
        </form>
      </div>
      ...
      <div className="rounded-lg border border-slate-750 bg-slate-900/90 p-5 shadow-sm space-y-4">
        <h2 className="text-xs font-semibold text-white uppercase tracking-wider flex items-center gap-2 font-mono">
          <OpsIcon name="mail" size={14} className="text-purple-400" />
          <span>Persona-Based Outreach Composer</span>
        </h2>
        <div className="rounded border border-amber-800/60 bg-amber-950/30 p-3 text-xs text-amber-200 flex items-center gap-2">
        ...
        <form method="get" className="flex flex-wrap gap-2.5">
          ...
          <button type="submit" className="ops-button-secondary text-xs">
            Generate Template
          </button>
        </form>

        <form action={sendOutreachEmails.bind(null, id)} className="space-y-3">
          <input
          ...
          <textarea
            ...
            className="ops-input w-full p-3 font-mono text-xs text-white"
          />
          {composedTemplate && (
            <div className="rounded-md border border-slate-800 bg-slate-950 p-4 font-mono text-xs text-slate-300">
          ...
          <fieldset className="rounded border border-slate-750 bg-slate-950/60 p-3">
              ...
              {contactRows.filter((contact) => contact.email).map((contact) => (
                <label key={contact.id} className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                  <input
                    ...
                    className="size-4 accent-blue-500 rounded"
                  />
                  <span className="truncate">{contact.full_name} &lt;{contact.email}&gt;</span>
                </label>
              ))}
              {!contactRows.some((contact) => contact.email) && (
                <p className="text-xs text-slate-400">Add a contact email before composing outreach.</p>
              )}
            </div>
          </fieldset>
          <label className="block font-mono text-[10px] font-semibold uppercase tracking-wider text-slate-400">
            ...
            <input
              type="datetime-local"
              name="scheduled_for"
              className="ops-input mt-1 block w-full text-xs sm:max-w-xs font-mono"
            />
          </label>
          <button
            type="submit"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <form action={logCallRemark.bind(null, id)} className="rounded-lg border border-slate-750 bg-slate-900/90 p-4 shadow-sm">
          <label className="flex items-center gap-2 font-mono text-xs font-semibold uppercase tracking-wider text-slate-300">
            <OpsIcon name="phone" size={14} className="text-blue-400" />
            Call remarks
          </label>
          <textarea name="remark" rows={3} required className="ops-input mt-2.5 w-full p-2.5 text-xs text-white" />
          <button type="submit" className="ops-button-secondary mt-2 text-xs">Log Call</button>
        </form>
        <form action={addSpcRemark.bind(null, id)} className="rounded-lg border border-slate-750 bg-slate-900/90 p-4 shadow-sm">
          <label className="flex items-center gap-2 font-mono text-xs font-semibold uppercase tracking-wider text-slate-300">
            <OpsIcon name="shield" size={14} className="text-amber-400" />
            SPC remarks
          </label>
          <textarea name="remark" rows={3} required className="ops-input mt-2.5 w-full p-2.5 text-xs text-white" />
          <button type="submit" className="ops-button-secondary mt-2 text-xs">Add Remark</button>
        </form>
      </div>

      {/* Committee Vault Files */}
      <div className="rounded-lg border border-slate-750 bg-slate-900/90 p-5 shadow-sm space-y-3">
        <h2 className="text-xs font-semibold text-white uppercase tracking-wider flex items-center gap-2 font-mono">
          <OpsIcon name="file-text" size={14} className="text-emerald-400" />
          <span>Placement Committee Vault</span>
        </h2>
        <form action={uploadVaultFile.bind(null, id)} className="flex flex-wrap sm:flex-nowrap items-center gap-2.5">
          <input
            type="file"
            name="file"
            required
            accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.jpg,.jpeg,.png"
            className="ops-input flex-1 text-xs text-slate-400 file:mr-2 file:rounded file:border-0 file:bg-slate-800 file:px-2.5 file:py-1 file:text-xs file:text-slate-200"
          />
          <button className="ops-button-primary text-xs shrink-0">
            Upload Vault File
          </button>
        </form>
        <ul className="divide-y divide-slate-800 rounded border border-slate-800 bg-slate-950/60">
          {(vaultFiles ?? []).map((file) => (
          ...
          {(vaultFiles ?? []).length === 0 && (
            <li className="p-4 text-xs font-mono text-slate-400 text-center">No vault files attached yet.</li>
          )}
        </ul>
      </div>
      ...
      <div className="rounded-lg border border-slate-750 bg-slate-900/90 p-5 shadow-sm space-y-3">
        ...
        <div className="space-y-3 font-mono text-xs">
          {activityRows.map((a) => {
            ...
            return (
              <div key={a.id} className="rounded border border-slate-800 bg-slate-950 p-3.5">
                ...
                {a.call_remarks && <p className="mt-1.5 text-slate-200 font-sans text-xs">Call: {a.call_remarks}</p>}
                {a.spc_remarks && <p className="mt-1 text-slate-200 font-sans text-xs">SPC: {a.spc_remarks}</p>}
                {a.previous_mails_summary && (
                  <p className="mt-1 whitespace-pre-wrap text-slate-300 font-sans text-xs">{a.previous_mails_summary}</p>
                )}
                {job && (
                  <div className="mt-2 rounded border border-blue-900/60 bg-blue-950/40 p-2.5 text-[10px] text-slate-300">
                    Delivery: <span className="font-semibold text-white">{job.status}</span>
                ...
                {a.channel === "email" && (
                  <form action={updateOutreachStatus.bind(null, id, a.id)} className="mt-2.5 flex items-center gap-2">
                    <select name="merge_status" defaultValue={a.merge_status ?? ""} className="ops-select text-[11px] py-1">
                      {MERGE_STATUSES.map((status) => (
                        <option key={status} value={status}>{status.replaceAll("_", " ")}</option>
                      ))}
                    </select>
                    <button type="submit" className="ops-button-secondary text-xs py-1 px-2.5 min-h-0">Update</button>
                  </form>
                )}
                {job && ["blocked", "failed"].includes(job.status) && (
                  <form action={retryOutreachNotification.bind(null, id, job.id)} className="mt-2">
                    <button type="submit" className="ops-button-secondary text-xs py-1 px-2.5 min-h-0">Retry delivery</button>
                  </form>
                )}
                {job && ["blocked", "queued", "scheduled", "failed"].includes(job.status) && (
                  <form action={rescheduleOutreachNotification.bind(null, id, job.id)} className="mt-2 flex flex-wrap gap-2">
                    <input type="datetime-local" name="scheduled_for" required className="ops-input text-xs py-1" />
                    <button type="submit" className="ops-button-secondary text-xs py-1 px-2.5 min-h-0">Reschedule</button>
                  </form>
                )}
              </div>
            );
          })}
          {activityRows.length === 0 && <p className="text-slate-400 font-mono text-center py-4">No outreach logs recorded yet.</p>}
        </div>
      </div>
    </div>
  );
