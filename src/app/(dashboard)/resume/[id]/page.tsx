...
export default async function CvViewerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-5 print:hidden">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono text-slate-500">
            <Link href={`/resume/studio?cv=${document.id}`} className="hover:text-slate-900 flex items-center gap-1 transition-colors">
              <OpsIcon name="file-text" size={13} />
              <span>Resume Studio</span>
            </Link>
            <span>/</span>
            <span className="text-slate-700">Verified CV Preview</span>
          </div>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-3">
            <span>{document.students?.name ?? content.personalInfo.name}</span>
            <span className="rounded bg-amber-50 px-2.5 py-0.5 font-mono text-xs font-semibold text-amber-700 border border-amber-200">
              {document.students?.roll_no}
            </span>
          </h1>
          <p className="mt-1 font-mono text-xs text-slate-500">
            Persona: <strong className="text-slate-700 font-medium">{document.company_type_personas?.category_name ?? "General Placement"}</strong> · Version {document.version_no}
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-slate-100 p-6 shadow-sm">
